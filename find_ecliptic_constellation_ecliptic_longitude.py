...
#
def calculate_zodiac_longitude():
    ...
    for constellation in boundary_constellation_names:
       ...
       # print(constellation)
       # print(current_coordinate.transform_to(GeocentricTrueEcliptic).to_string(style='decimal').split()[0])
        zodiac_longitudes[constellation] = float(current_coordinate.transform_to(GeocentricTrueEcliptic).to_string(style='decimal').split()[0])
    ...
    #    print('{}: ({})\n'.format(astronomical_symbols[constellation], boundary_coordinates[index]))
        index += 1
    return zodiac_longitudes


if __name__ == '__main__':
    output = calculate_zodiac_longitude()
    print(output)
